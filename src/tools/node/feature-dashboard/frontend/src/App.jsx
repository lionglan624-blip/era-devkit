                    ...
                    <div className="header-commands">
                        ...
                        >
                            <div className="status-indicators">
                                ...
                                <span
                                    className={`status-item ${!connected ? 'error' : ''}`}
                                    title="WebSocket"
                                >
                                    WS {connected ? '●' : '✗'}
                                </span>
                                ...
                                {(healthStatus.ccsProfiles?.length > 0
                                    ...
                                    const rateClass =
                                        data?.weekly?.percent >= 90 || data?.session?.percent >= 90
                                            ? 'rate-critical'
                                            : data?.weekly?.percent >= 70 ||
                                                data?.session?.percent >= 70
                                              ? 'rate-warning'
                                              : '';
                                    const weeklyReset =
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span className="rate-values">
                                                W:{data?.weekly ? `${data.weekly.percent}%` : '-'}{' '}
                                                S:
            ...
            {!showExecutionPanel && (
                <button className="floating-panel-btn" onClick={() => setShowExecutionPanel(true)}>
                    ...
                    <span className="fpb-label">
                        {runningFeatures.size > 0
                            ? `${runningFeatures.size} running`
                            : 'Executions'}
                    </span>
                </button>
            )}
