...
全スラッシュコマンドが一貫した構造パターンを持つ: 明確な Phase 定義、明示的な subagent 契約、エラーハンドリング、Recovery Procedures。これにより予測可能な実行と保守性の向上を実現する。また、Design ドキュメントは明確な粒度基準を持ち、1 Version = 1 Design の原則に従う。

### Problem (Current Issue)

Design 粒度に以下の問題が存在 (F233 Issue Inventory より):

| ID | 問題 | 現状 | リスク |
|:--:|------|------|--------|
| DS1 | 1 Version : N 機能問題 | v2.x-v5.x が1 Designに混在 (ntr-core-system.md) | 粒度不整合 |
| DS5 | Architecture Document 未分離 | 上位設計と個別設計が混在 | 責務不明確 |

### Goal (What to Achieve)

Design ドキュメントの粒度標準を確立:
1. 1 Design = 1 Version の原則を文書化
2. designs/ と reference/ の役割分離を明確化
3. designs/ntr-core-system.md を reference/ntr-core-overview.md として移動（Version Roadmap セクションのみ削除、他のセクションは保持）
...
|:---:|-------------|------|--------|---------|----------|:------:|
| 1 | 粒度基準文書化 | code | Grep | contains | "1 Design = 1 Version" | [x] |
| 2 | designs/reference 役割定義 | code | Grep | contains | "実装設計" | [x] |
| 3 | ntr-core → reference移動 | file | Glob | exists | Game/agents/reference/ntr-core-overview.md | [x] |
| 4 | Version詳細削除確認 | code | Grep | not_contains | "Version Roadmap" | [x] |
| 5 | ビルド成功 | build | dotnet | succeeds | - | [x] |

### AC Details

**AC#1 Test**: `Grep pattern="1 Design = 1 Version" path="Game/agents/designs/README.md"`
**AC#1 Expected**: Pattern found in designs/README.md (file will be updated by Task#1)

**AC#2 Test**: `Grep pattern="実装設計" path="Game/agents/designs/README.md"`
**AC#2 Expected**: Role distinction pattern found (file will be updated by Task#2)

**AC#3 Test**: `Glob("Game/agents/reference/ntr-core-overview.md")`
**AC#3 Expected**: File exists

**AC#4 Test**: `Grep pattern="Version Roadmap" path="Game/agents/reference/ntr-core-overview.md"`
