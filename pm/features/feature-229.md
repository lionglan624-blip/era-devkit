...
| P1 | IMPLE_FEATURE_ID created as `c:Eraera紁E��館protoNTR.gitIMPLE_FEATURE_ID` in root | do.md L203 uses relative path, CWD-dependent |

| P3 | Test logs scattered in root directory (`*.log`) | Output location not unified |



**Note**: Malformed file `c:Eraera紁E��館protoNTR.gitIMPLE_FEATURE_ID` no longer exists (verified 2025-12-27).
...
6. Unify test log output location to `.tmp/` or `_out/logs/debug/`
...
python src/tools/python/verify-logs.py --dir _out/logs/prod --scope regression
...
python src/tools/python/verify-logs.py --dir _out/logs/prod --scope feature:188
...
**Note**: AC8 scope is depth 1 only. Logs in `engine/` and `pm/logs/` are **out of scope** for F229 (potential Quick Win for future cleanup).
...
**Location**: `src/tools/python/verify-logs.py:32`
...
**Evidence**: Existing file `c:Eraera紁E��館protoNTR.gitIMPLE_FEATURE_ID` in root directory.



**Root cause**: Command executed when CWD was `c:\Era\era紁E��館protoNTR`, causing `.git/` to be interpreted as filename prefix.
...
**Proposed unified location**: `.tmp/test-*.log` (temporary outputs) or `_out/logs/debug/` (debug logs)
...
| 2025-12-27 | update | FL | Type: infra ↁEerb | Implementation required (AC5,6,8) |
...
1. ~~Remove malformed file~~ ✁EAlready removed (verified 2025-12-27)

2. Move any root `*.log` files to `.tmp/` or `_out/logs/debug/`
