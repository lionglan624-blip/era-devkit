...
IF context_pct >= 80:
    ...
    # [pending] items are already persisted in feature.md Review Notes
    batch_mode = true
    → Skip Step 2 remainder (no AskUserQuestion)
    → Skip Step 3 (Dependency Propagation)
    → Skip Step 5 (Skill Update)
    → Skip Step 6 (Philosophy Gate)
    → Execute Step 7 (Status Update) with NO status promotion
    → Execute Step 8 (Index Sync) — priority reorder only, no status change
    → GOTO Step 9 (Report) with "Context Pressure" template
ELSE:
    batch_mode = false
...
### Context Pressure (Batch Completion)
```
=== FL一時停止（コンテキスト圧力）: {target_type} {target_name} ===
イテレーション: {N}/{MAX} | 自動修正: {count}件
コンテキスト使用率: {context_pct}% (閾値: 80%)
未解決pending: {unresolved_pending.count}件
  - {issue}
  - ...

ステータス: 変更なし（{current_status}のまま）

→ 新セッションで `/fl {ID}` を再実行してください（pending項目はfeature.mdに保存済み）
```
