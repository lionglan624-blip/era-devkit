...
F223 で洗い出された問題�EぁE��、本 Feature は `/do` ワークフロー自体�E堁E��性を担保する、E
...
F223 Category 2 (Workflow Ambiguity) で発見された 14 の問顁E
...
| W8 | 7-9 | Phase 7-9 の役割ぁEdo.md で不�E確 | CI委任 vs 手動実行�E刁E��刁E��忁E��E|

| W9 | pre-commit | 最終防波堤として AC+Regression+verify 忁E��E| LLM スキチE�E対筁E|

| W10 | 10 | Phase 10 頁E��不�E確 | Regression→verify→Post-Review→Report頁E���E記忁E��E|

| W11 | 7-10 | AC→Regression 頁E��E| 一般原則はRegression→AC(既存保護優允E。現設計�E送E|

| W12 | 10 | Post-Review 忁E��ゲート未明訁E| READYを返さなぁE��User Approvalに進めなぁE��とが不�E確 |

| W13 | 10 | Post-Review 異常時対応なぁE| UNREACHABLE/IRRELEVANT時�E対応未定義 |

| W14 | 10 | Post-Review 判断絶対視未記輁E| NEEDS_REVISION無視して進める余地あり |

| W15 | 全佁E| Phase番号体系の不整吁E| Step小数点が一部のみ、Phase7-9がCI委任で/do冁E��の役割不�E確 |



現状の do.md は Phase 1-10 構�Eだが、Phase 7-9 ぁECI に委任されており、Phase 10 は Post-Review ↁEReport ↁEApproval ↁEFinalize の 4 スチE��プを "10.0", "10.1" で表現しており不�E瞭、E
...
1. **Phase 体系の再設訁E(W15)**: 小数点廁E��、Phase 7-9 統合、Phase 10 めE3 つに刁E�� ↁE明確な 9 Phase 体系へ

2. **Timeout & Polling 修正 (W1-W3)**: kojo-writer の polling に上限設定、status file path 統一、per-writer timeout 追加

3. **Failure Counter 実裁E(W6)**: 3 連続失敗で STOP する仕絁E��を�E斁E��

4. **Post-Review ゲート強匁E(W12-W14)**: READY 忁E��、NEEDS_REVISION/UNREACHABLE/IRRELEVANT 時�E対応を明訁E
5. **Regression 頁E��修正 (W11)**: AC より Regression を�Eに実行（既存保護優先！E
6. **pre-commit Hook 最終防波堤匁E(W9)**: verify-logs.py による AC+Regression 検証を�E訁E
...
| 7 | Status file path documented | file | Grep | contains | "pm/status/{ID}_K{N}.txt" | [x] |
...
**Test**: `Grep("pm/status", path: ".claude/commands/do.md", output_mode: "content")`
...
| 6 | Verification (Regression→AC→Debug) | Hooks (BOM/Build/Warnings) + Regression + AC + Debug loop (max 3) | 6-9 統吁E|
...
sleep 360 ↁEGlob ↁE(不足なめEsleep 60 繰返し)
...
- Status file path: `pm/status/{ID}_K{N}.txt` (absolute path)
...
2. Glob("pm/status/{ID}_K*.txt")

3. If count < 10:

   - Check elapsed time since dispatch

   - If > max_polling_cycles * 60: **STOP** ↁEReport timeout

   - For each missing K{N}, check if > per_writer_timeout: **STOP** ↁEReport stuck writer
...
**Status File Path Standard**: `pm/status/{ID}_K{N}.txt` (use absolute path in Glob)
...
2. Report to user: "3回連続で{type}失敗しました。別アプローチを試しますか�E�E
...
| READY | **Gate PASS** ↁEProceed to Phase 8 |

| NEEDS_REVISION | **Gate BLOCK** ↁEFix issues ↁEre-run Phase 7 |

| UNREACHABLE | **STOP** ↁEReport to user: "Feature {ID} のゴールが達成不可能です。スコープ�E定義が忁E��です、E |

| IRRELEVANT | **STOP** ↁEReport to user: "Feature {ID} の実裁E�E容がゴールと無関係です。Tasks を見直してください、E |
...
**Loop Limit**: Max 3 NEEDS_REVISION cycles. On 4th, **STOP** ↁEReport to user.
...
- **STOP** ↁEThis should never happen (pre-commit hook should have blocked)

- Report to user: "verify-logs.py で失敗検�E。pre-commit hook が機�EしてぁE��ぁE��能性があります、E

- Do NOT proceed to Step 8.2



**CRITICAL**: This is the **final mechanical gate** before user approval. LLM 主観判断を排除し、機械皁E��証のみを信頼する、E
```
...
| 2025-12-27 | Fix | opus | Phase 10ↁE/9 refs | SUCCESS |
...
- [do.md](../../../archive/claude_legacy_20251230/commands/do.md) - Target file for this feature
...
W4 ("CI で検証済み" but hook 未確誁E is assigned to **F229 (Infrastructure Verification)**, not F227.
...
- W5 (User approval case-insensitivity) ↁEF231 (User Approval UX)

- W7 (Error recovery procedures) ↁEF230 (Recovery Procedures)

- Implementation of verify-logs.py fixes (I5) ↁEF229 (Infrastructure Verification)

- P1, P3 (file placement) ↁEF229 (Infrastructure Verification)
