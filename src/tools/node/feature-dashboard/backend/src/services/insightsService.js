...
export class InsightsService {
  ...
  async capture({ sendEmail = true } = {}) {
    ...
    try {
      ...
      claudeLog.info(`[Insights] Capture complete: duration=${Math.round(duration / 1000)}s, success=${result.success}, reason=${result.reason}`);
  ...
  async _runCapture(profile, reportPath, beforeMtime) {
    ...
    return new Promise((resolve) => {
      ...
      const finish = (success, reason) => {
        ...
        claudeLog.info(`[Insights] Finishing: success=${success}, reason=${reason}, chunks=${chunkCount}`);
        // Log last chunks at debug for diagnostics
        const lastChunks = rawChunks.slice(-10);
        for (let i = 0; i < lastChunks.length; i++) {
          claudeLog.debug(`[Insights] Chunk[-${lastChunks.length - i}]: ${lastChunks[i].substring(0, 300).replace(/\n/g, '\\n')}`);
        }
      ...
      ptyProcess.onData((data) => {
        ...
        if (insightsSent && REPORT_READY_PATTERN.test(data)) {
          claudeLog.info(`[Insights] PTY output: "report is ready" detected at chunk ${chunkCount}`);
