...
2. **TALENT isolation bug**: Default setup sets `TALENT:諱倶ｺｺ+諱区・` for all characters, preventing isolated testing of individual TALENT branches (諱区・/諤晄・/縺ｪ縺・

### Goal
Fix both issues to enable proper kojo test execution.

### Context
- Discovered in: Feature 135 (COM_20 繧ｭ繧ｹ縺吶ｋ)
...
| 2 | TALENT isolation (莠ｺ螯ｻ:15) | output | contains | "TALENT:TARGET:15=1" | [x] |
| 3 | TALENT isolation (繝・Φ繝・Ξ:14) | output | contains | "TALENT:TARGET:14=1" | [x] |
| 4 | TALENT isolation (縺ｪ縺・ | output | not_contains | "TALENT:TARGET:16" | [x] |
| 5 | Build succeeds | build | succeeds | - | [x] |

> **Note**: AC#2-3 originally mislabeled as 諱区・/諤晄・. Corrected to match actual TALENT indices (see Talent.csv).
...
| 2 | 2 | Change TALENT setup to isolate 諱区・ state from JSON only (no defaults) | [x] |
| 3 | 3 | Change TALENT setup to isolate 諤晄・ state from JSON only (no defaults) | [x] |
| 4 | 4 | Change TALENT setup to isolate 縺ｪ縺・state from JSON only (no defaults) | [x] |
...
- Location: `engine/uEmuera/KojoTestMode.cs` (probable)
- Current behavior: `[KojoTest] Set TALENT:諱倶ｺｺ+諱区・ for K1-K10 characters`
