...
- Check Tasks and ACs status:
  - **Guard**: Any status other than `[x]`, `[ ]`, `[-]`, `[B]` → return `NOT_READY` immediately with error: "Invalid AC status detected: {value}. Valid: [ ], [x], [-], [B]. Revise the AC to verify the current state."
