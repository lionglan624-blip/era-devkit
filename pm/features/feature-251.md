...
index-features.md の Phase 8 Summary は kojo-mapper から自動生成されるべき。手動更新は SSOT 違反。

### Problem (Current Issue)
- F240 で「Phase 8 Summary 削除は別 Feature で実装予定（kojo-mapper に COM カテゴリ別進捗機能を追加後）」と延期
- F166/F167 で kojo-mapper に 14 カテゴリ検出機能は実装済み
- しかし Phase 8 Summary の自動化/削除 Feature は未作成
- index-features.md の Phase 8 Summary は手動更新のまま（SSOT 違反継続中）

### Goal (What to Achieve)
1. index-features.md から手動 Phase 8 Summary を削除
2. kojo-mapper --coverage 出力を SSOT として使用
3. `/plan`、`/next`、`/do` に kojo-mapper --coverage への参照を追加

### Session Context
- **Origin**: F240 延期事項の正式 Feature 化
- **Dependencies**: F166 (14カテゴリ検出), F167 (検証完了)
...
| 2 | kojo-mapper 参照リンク追加 | file | Grep | contains | "tools/kojo-mapper/kojo_mapper.py" | [x] |
| 3 | content-roadmap.md 進捗リンク更新 | file | Grep | contains | "tools/kojo-mapper" | [x] |
| 4 | content-roadmap.md 古いリンク削除 | file | Grep | not_contains | "#phase-8-サマリー" | [x] |
| 5 | kojo-mapper --coverage 実行成功 | output | bash | contains | "=== Kojo Coverage Report ===" | [x] |
| 6 | Current Phase セクション保全 | file | Grep | contains | "## Current Phase: 8d" | [x] |
| 7 | Links セクション保全 | file | Grep | contains | "## Links" | [x] |
...
**AC1**: index-features.md から `## Phase 8 Summary` セクション（テーブル含む）を削除

**AC2**: index-features.md に kojo-mapper 実行コマンドへの参照を追加
```markdown
**Progress**: `python tools/kojo-mapper/kojo_mapper.py "Game/ERB/口上" --coverage`
```

**AC3**: content-roadmap.md の "Progress tracking" リンクを更新

**AC4**: content-roadmap.md から古い `#phase-8-サマリー` アンカーリンクを削除
...
**AC6**: index-features.md の `## Current Phase: 8d` セクションが保全されていること

**AC7**: index-features.md の `## Links` セクションが保全されていること
...
|:-----:|:---:|-------------|:------:|
| 1 | 1 | index-features.md から ## Phase 8 Summary セクション（テーブル含む）を削除 | [○] |
| 2 | 2 | index-features.md の ## Current Phase セクション後に Progress Tracking セクション挿入 | [○] |
| 3 | 3 | content-roadmap.md line 234 の Progress tracking リンクを tools/kojo-mapper/ に変更 | [○] |
| 4 | 4 | content-roadmap.md line 234 から #phase-8-サマリー アンカーを削除 | [○] |
| 5 | 5 | kojo-mapper --coverage 実行して出力を確認 | [○] |
| 6 | 6 | index-features.md に ## Current Phase: 8d が残っていることを確認 | [○] |
| 7 | 7 | index-features.md に ## Links が残っていることを確認 | [○] |
| 8 | 8 | plan.md Phase 1.1 (Read Current State) セクションに kojo-mapper --coverage 実行ステップ追加 | [○] |
| 9 | 9 | next.md Priority 2 セクションの Phase 8 サマリー参照を kojo-mapper に置換 | [○] |
...
- [feature-240.md](feature-240.md) - 延期元
- [feature-166.md](feature-166.md) - kojo-mapper 14カテゴリ
- [feature-167.md](feature-167.md) - 検証完了
