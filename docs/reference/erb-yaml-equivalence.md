...
プロファイルは `tools/KojoComparer/state-profiles.json` で外部設定化済み。追加は JSON 編集のみ。
...
**場所**: `tools/ErbParser/ErbParser.cs`
...
**場所**: `tools/KojoComparer/FileDiscovery.cs:177`
...
| `tools/ErbToYaml/FileConverter.cs` | com_id出力追加 |
| `tools/ErbToYaml/ComIdInjector.cs` | 新規: com_id注入ツール |
| `tools/ErbToYaml/Program.cs` | --inject-com-idモード追加 |
| `tools/KojoComparer/FileDiscovery.cs` | com_idベースのルックアップに書き換え |
| `tools/KojoComparer/KojoBranchesParser.cs` | ComIdプロパティ追加 |
| `tools/KojoComparer/KojoYamlParser.cs` | ComIdプロパティ追加 |
| `tools/KojoComparer/YamlRunner.cs` | サフィックスなしYAMLパス対応 |
| `tools/KojoComparer/OutputNormalizer.cs` | 会話イントロパターン除去追加、PRINTFORMW結合分割 |
| `tools/KojoComparer/BatchExecutor.cs` | サブ関数テスト名生成対応 |
| `tools/KojoComparer/ErbEvaluator.cs` | 新規: 軽量インプロセスERBインタープリター |
| `tools/KojoComparer/ErbBatchEvaluator.cs` | 新規: ErbEvaluatorベースのバッチ実行 |
| `tools/kojo-mapper/com_file_map.json` | COM_300-316範囲分割, K10 COM_091修正, COM_463 sub_functions追加 |
| `tools/YamlSchemaGen/dialogue-schema.json` | com_idフィールド追加 |
| `Game/YAML/Kojo/5_レミリア/K5_会話親密_9.yaml` | 欠落PRINTFORML行追加 |
| `tools/ErbParser/ErbParser.cs` | #DIM変数宣言追跡 + DIM変数代入認識 (Bug 7) |
...
dotnet run --project tools/KojoComparer/ -- --all --multi-state

# 等価性テスト全件実行 (旧headlessエンジン, ~8分)
dotnet run --project tools/KojoComparer/ -- --all --legacy
...
# バッチ変換（com_id付き）
dotnet run --project tools/ErbToYaml/ -- --batch Game/ERB/口上/ Game/YAML/Kojo/

# com_id注入（既存YAMLに注入）
dotnet run --project tools/ErbToYaml/ -- --inject-com-id Game/ERB/口上/ Game/YAML/Kojo/
