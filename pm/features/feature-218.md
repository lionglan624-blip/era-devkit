...
├── ac/           ↁEFeature AC logs (accumulated)

├── regression/   ↁEFlow test logs

└── engine/       ↁESeparate location for engine tests (PROBLEM)
...
━E  ├── engine/       ↁEEngine AC (unified)

━E  └── feature-*/    ↁEFeature AC

└── regression/       ↁESystem AC (flow tests)
...
2. **Context-aware verification scope**:
   ...
   - `/do {ID}` ↁEFeature AC + Regression

   - General commit ↁERegression only

   - Engine change ↁEEngine AC + Regression
...
| 1 | Engine tests output to unified log dir | file | static | exists | _out/logs/prod/ac/engine/test-result.trx | [ ] |
...
- Current: `_out/logs/prod/engine/test-result.trx`

- New: `_out/logs/prod/ac/engine/test-result.trx`
...
python src/tools/python/verify-logs.py --dir _out/logs/prod --scope feature:217



# All (full audit)

python src/tools/python/verify-logs.py --dir _out/logs/prod --scope all
...
- Command: `python src/tools/python/verify-logs.py --help`
...
- Command: `python src/tools/python/verify-logs.py --dir _out/logs/prod --scope regression`
...
- Command: `python src/tools/python/verify-logs.py --dir _out/logs/prod --scope feature:217`
